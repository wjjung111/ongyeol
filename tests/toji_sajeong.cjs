...
(async()=>{
  ...
  const opinion=await page.evaluate(async()=>{
    ...
    return {saj,area:maps.parcels[0]['토지_사정면적'],gongbu:maps.parcels[0]['토지_면적'],
            ...
            bld:{공부:maps.buildings[0]['건물_면적'],사정:maps.buildings[0]['건물_사정면적']},
  ...
  assert.equal(opinion.area,'111.85','사정면적 칸은 숫자만(공부면적 칸이 따로 있음)');
  assert.equal(opinion.gongbu,'223.7','물건 표는 공부면적 그대로');
  assert.ok(opinion.cells.filter(t=>t==='111.85').length>=3,'물건 표·시산가액 두 표의 사정면적 칸');
  assert.ok(opinion.cells.filter(t=>t==='223.7').length>=3,'같은 표들의 공부면적 칸');
  ...
  // 사정면적 머리: 물건 토지·물건 건물·공시·거래·건물가액·최종 6곳 / 공부면적 머리: 물건 토지·물건 건물·공시·거래·재조달원가 5곳
  assert.equal(opinion.heads.filter(h=>h.includes('사정면적')).length,6);
  assert.equal(opinion.heads.filter(h=>h.includes('공부면적')).length,5);
  assert.equal(opinion.bld.공부,'200','건물 물건 표는 공부(연)면적 합계');
  assert.equal(opinion.bld.사정,'60','건물 물건 표 사정면적 합계');

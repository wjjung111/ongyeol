/*
  ...
  - 온결 감정평가사만 쓰는 화면(허브·집합건물·입주권·토지건물·토지조회·개발허브)에 공통으로 붙는다.
    (건축물대장·시점수정 계산기는 접속코드 대신 이름만 받는 arap_name.js를 쓴다)
  ...
  - arm_시점수정.html·건축물대장.html(이름 게이트)과 관리자.html(수신함이 비밀번호 검증)에는 붙이지 않는다.

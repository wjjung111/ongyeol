...
# 매매가격지수 통계표 (R-ONE STATBL_ID, 고정값) — 지역코드(CLS_ID)를 하드코딩하지 않고
# 표 전체를 페이지로 받아 CLS_FULLNM 첫 구간(시도)이 roots에 드는 지역만 저장한다.
...
function Fetch-IndexData {
  ...
  foreach ($kind in $tables.Keys) {
    ...
    if (-not $gotRows) { Write-Host ("    {0} 자료없음(월간 미공표?) — 이 표는 생략" -f $kind) -ForegroundColor Yellow; continue }
    # 표시명 = 마지막 구간. 시도 간 동명(예: 서울 중구/인천 중구) 충돌 시 '시도 이름'으로 구분
    $regions = [ordered]@{}
    foreach ($full in $byFull.Keys) {
      $parts = $full -split ">"
      $name = $parts[-1].Trim()
      if ($regions.Contains($name)) { $name = "{0} {1}" -f $parts[0].Trim(), $parts[-1].Trim() }
